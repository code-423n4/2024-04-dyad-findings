...
| --- | --- | --- |
| \[L-01\] | Code does not follow the best practice of check-effects-interaction | Low |
| \[L-02\] | Calls inside a loop | Low |
| \[L-03\] | ownable2 should be used instead of ownable. | Low |
| \[L-04\] | Insufficient coverage | Low |
| \[N-05\] | NatSpec: functions \`@param\` ,\`@notice\`, \`@title\`, \`@dev\`, tags are missing | Non-Critical |
| \[N-05\] | Natspec @author is missing | Non-Critical |
| \[N-06\] | NatSpec: Functions `@return` tag is missing | Non-Critical |
| \[N-07\] | In functions which accept an address as a parameter, there should be a zero address check to prevent bugs | Non-Critical |
| \[N-08\] | Revert statements within external and public functions can be used to perform DOS attacks | Non-Critical |
| \[N-09\] | Private and internal state variables should have a preceding _ in their name unless they are constants | Non-Critical |
| \[N-10\] | Contract lines should not be longer than 120 characters for readability | Non-Critical |
| \[N-11\] | Functions within contracts are not ordered according to the solidity style guide | Non-Critical |
| \[N-12\] | Interface imports should be declared first | Non-Critical |
| \[N-13\] | Consider using named mappings | Non-Critical |
| \[N-15\] | Style guide: Contract does not follow the Solidity style guide's suggested layout ordering | Non-Critical |
| \[N-16\] | Constructors missing validation | Non-Critical |
| \[N-17\] | Use a single contract or library for system wide constants | Non-Critical |
| \[N-18\] | Consider using SMTChecker | Non-Critical |
| \[N-19\] | \`require()\`/ \`revert()\` statements should have descriptive reason strings | Non-Critical |
| \[N-20\] | Contracts should have full test coverage | Non-Critical |
| \[N-21\] | Consider adding formal verification proofs | Non-Critical |
| \[N-22\] | Lack Of Brace Spacing | Non-Critical |
| \[N-23\] | If statement control structures do not comply with best practices | Non-Critical |
| \[N-24\] | Missing events in sensitive functions | Non-Critical |
| \[N-25\] | Consider using a format prettier or forge fmt | Non-Critical |
| \[N-26\] | Consider validating all user inputs | Non-Critical |
| \[N-27\] | Consider using named returns | Non-Critical |
| \[N-28\] | Errors should have parameters | Non-Critical |
| \[N-29\] | Variables need not be initialized to zero | Non-Critical |
| \[N-30\] | Consider adding emergency-stop functionality | Non-Critical |
| \[N-32\] | Generate perfect code headers every time | Non-Critical |
| \[N-32\] | Use a more recent version of Solidity | Non-Critical |
| \[N-33\] | For modern and more readable code; update import usage | Non-Critical |
| \[N-34\] | Pragma version^0.8.17 version is too recent to be trusted. | Non-Critical |
